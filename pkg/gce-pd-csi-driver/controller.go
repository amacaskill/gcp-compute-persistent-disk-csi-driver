...
func (gceCS *GCEControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if content != nil {
		...
		if content.GetVolume() != nil {
			...
			if err != nil {
				return nil, status.Error(codes.InvalidArgument, fmt.Sprintf("CreateVolume Volume ID is invalid: %v", err))
			}
			...
			if err := gce.ValidateDiskParameters(diskFromSourceVolume, params); err != nil {
				return nil, status.Errorf(codes.InvalidArgument, `CreateVolume source volume parameters must match CreateVolumeRequest Parameters %v`, params)
			}
			// Verify the name of the clone is different than the source disk.
			if volKey.Name == req.Name {
				return nil, status.Errorf(codes.AlreadyExists, `CreateVolume source volume Name cannot be the same as CreateVolumeRequest Name %s`, req.Name)
			}
		}
	}
	// Create the disk
